...
orderRouter.post('/order', auth, async (req, res) => {
    ...
    try{
        await order.save()
        const url = `https://ipay.arca.am/payment/rest/register.do?userName=${process.env.PAYMENT_LOGIN}&password=${process.env.PAYMENT_PASSWORD}&returnUrl=http://www.amzbamboo.com/order/callback&amount=10&orderNumber=${order._id}&currency=4217`
